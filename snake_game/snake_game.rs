...
impl<'a, 'sdl> SnakeGame<'a, 'sdl> {
    fn load_snake_game(&mut self) {
        let game_code: Vec<u8> = vec!(0x20, 0x06, 0x06, 0x20, 0x38, 0x06, 0x20, 0x0d, 0x06, 0x20, 0x2a, 0x06, 0x60, 0xa9, 0x02, 0x85,
    ...
    fn update_screen_state(&mut self) -> bool {
        ...
        for i in SCREEN_START..SCREEN_START + 0x0400 {
            ...
            let (r, g, b) = color.rgb();
            if r != self.screen_state[frame_idx] || g != self.screen_state[frame_idx + 1] || b != self.screen_state[frame_idx + 2] {
                update = true;
